...
int detect(char *model_path, char *image_path, char *save_image_path)
{
    ...
    LPRNet PostProcess;
    std::string LicenseResult;
    PostProcess.GetRecongitionResult(pblob, out_zps, out_scales, LicenseResult);
    std::cout << LicenseResult << std::endl;

    gettimeofday(&stop_time1, NULL);
    printf("postprocess run use %f ms\n", (__get_us(stop_time1) - __get_us(stop_time)) / 1000);

    cv::Mat PuttextImage = cv::Mat::zeros(img_height, img_width, src_image.type());
    putText(PuttextImage, LicenseResult, cv::Point(0, int(img_height / 2)), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255), 2);
    cv::Mat ResultImage;
    cv::vconcat(src_image, PuttextImage, ResultImage);
    cv::imwrite(save_image_path, ResultImage);
